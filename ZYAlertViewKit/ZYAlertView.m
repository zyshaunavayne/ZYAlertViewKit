...
/// 默认的标题字体
#define ZYAlertViewDefaultTitleFont [UIFont systemFontOfSize:18 weight:UIFontWeightRegular]
/// 默认的消息字体
#define ZYAlertViewDefaultMessageFont [UIFont systemFontOfSize:16 weight:UIFontWeightRegular]
/// 默认的标题颜色
#define ZYAlertViewDefaultTitleColor UIColor.blackColor
/// 默认的消息颜色
#define ZYAlertViewDefaultMessageColor [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:1.0]

@interface ZYAlertTextFieldMaker (Inner)
- (id)initWithTextField:(UITextField *)textField
            contentView:(UIView *)contentView
               mainView:(UIStackView *)mainView;
@end

@interface ZYAlertStackView : UIStackView
/// 分割线颜色
...
@implementation ZYAlertStackView
...
@interface ZYAlertAction ()
@property (nonatomic, copy) void (^handler)(ZYAlertAction *);
@property (nonatomic, copy) BOOL (^canClose)(ZYAlertAction *);
@property (nonatomic, assign) NSInteger setTitleColorCount;
@end

@implementation ZYAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(ZYAlertActionStyle)style handler:(void (^)(ZYAlertAction *))handler
{
    return [self actionWithTitle:title style:style handler:handler canClose:nil];
}

+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image style:(ZYAlertActionStyle)style
{
                       ...
                       canClose:(BOOL (^)(ZYAlertAction *action))canClose
{
    ...
    switch (style) {
        case ZYAlertActionStyleDefault:
...
@property (nonatomic, copy) NSArray <ZYAlertAction *>*alertActions;
@property (nonatomic, copy) NSArray <UITextField *> *addTextFields;
@property (nonatomic, strong) ZYAlertStackView *contentStackView;
@property (nonatomic, strong) ZYAlertStackView *actionsStackView;
@property (nonatomic, strong) ZYAlertStackView *bottomActionsStackView;
@property (nonatomic, strong) ZYAlertStackView *textFieldStackView;
@property (nonatomic, strong) ZYAlertStackView *customViewStackView;
@property (nonatomic, copy) NSArray <NSAttributedString *>*attributedContents;
@property (nonatomic, assign) ZYAlertViewStyle preferredStyle;
...
{
    ZYAlertView *alertView = [[ZYAlertView alloc] initWithTitle:title message:message preferredStyle:preferredStyle];
    return alertView;
}
...
+ (instancetype)alertViewWithTitle:(NSString *)title
                attributedMessages:(NSArray<NSAttributedString *> *)messages
                    preferredStyle:(ZYAlertViewStyle)preferredStyle
{
    ZYAlertView *alertView = [[ZYAlertView alloc] initWithTitle:title attributedMessages:messages preferredStyle:preferredStyle];
    return alertView;
}
               ...
               preferredStyle:(ZYAlertViewStyle)preferredStyle
{
    if (self = [super init]) {
        ...
        if (message && message.length > 0) {
            self.attributedContents = @[[[NSMutableAttributedString alloc] initWithString:message attributes:@{NSFontAttributeName: ZYAlertViewDefaultMessageFont,NSForegroundColorAttributeName: ZYAlertViewDefaultMessageColor}].copy];
        }
        self.preferredStyle = preferredStyle;
        self.titleColor = ZYAlertViewDefaultTitleColor;
        self.titleFont = ZYAlertViewDefaultTitleFont;
        _message = [message copy];
        _messageColor = ZYAlertViewDefaultMessageColor;
        _messageFont = ZYAlertViewDefaultMessageFont;
...
{
    if (self.preferredStyle == ZYAlertViewStyleActionSheet || self.preferredStyle == ZYAlertViewStyleChooseSheet) {
...
- (void)setupButtonLayout:(UIButton *)button actionStyle:(ZYAlertActionStyle)style imagePadding:(CGFloat)imagePadding
{
...
- (UIButton *)createActionButton:(ZYAlertAction *)action
{
    ...
    if (action.style == ZYAlertActionStyleCenterImageLeft || action.style == ZYAlertActionStyleCenterImageRight) {
...
- (void)updateContentStackView
{
    ...
    if (self.preferredStyle == ZYAlertViewStyleActionSheet) {
        self.contentStackViewConstraintTop.constant = self.attributedContents.count > 0 ? 12 : 0;
    } else if (self.preferredStyle == ZYAlertViewStyleChooseSheet) {
        self.contentStackViewConstraintTop.constant = self.attributedContents.count > 0 ? 12 : 0;
    } else {
...
- (void)updateTitleContentView
{
    ...
    /// 添加额外的按钮
    if (self.preferredStyle == ZYAlertViewStyleChooseSheet) {
...
{
    ZYAlertAction *action = self.alertActions[sender.tag];
    if (action.handler) {
...
- (void)addAction:(ZYAlertAction *)action
{
    ...
    if (self.preferredStyle == ZYAlertViewStyleActionSheet || self.preferredStyle == ZYAlertViewStyleChooseSheet) {
        ...
        for (ZYAlertAction *action in self.alertActions) {
            if (action.style == ZYAlertActionStyleCancel) {
                cancelStyleCount ++;
            }
        }
        if (action.style == ZYAlertActionStyleCancel) {
            NSAssert(cancelStyleCount == 0, @"`ZYAlertViewStyleActionSheet`样式下按钮Action `ZYAlertActionStyleCancel`只能存在一个");
            /// 修改默认颜色值
    ...
    CGFloat height = self.preferredStyle == (ZYAlertViewStyleActionSheet || ZYAlertViewStyleChooseSheet) ? 49 : 44;
    
    if (self.alertActions.count > 2 || self.preferredStyle == ZYAlertViewStyleActionSheet) {
...
{
    for (ZYAlertAction *action in actions) {
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField *textField, UIView *contentView, ZYAlertTextFieldMaker *make))configurationHandler
{
    ...
    if (configurationHandler) {
        ZYAlertTextFieldMaker *make = [[ZYAlertTextFieldMaker alloc] initWithTextField:textField contentView:contentView mainView:self.textFieldStackView];
...
- (void)show
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[ZYAlertView contentView] addSubview:self];
...
- (ZYAlertStackView *)contentStackView
{
    if (_contentStackView == nil) {
        _contentStackView = ZYAlertStackView.alloc.init;
...
- (ZYAlertStackView *)actionsStackView
{
    if (_actionsStackView == nil) {
        _actionsStackView = ZYAlertStackView.alloc.init;
...
- (ZYAlertStackView *)bottomActionsStackView
{
    if (_bottomActionsStackView == nil) {
        _bottomActionsStackView = ZYAlertStackView.alloc.init;
...
- (ZYAlertStackView *)textFieldStackView
{
    if (_textFieldStackView == nil) {
        _textFieldStackView = ZYAlertStackView.alloc.init;
...
- (ZYAlertStackView *)customViewStackView
{
    if (_customViewStackView == nil) {
        _customViewStackView = ZYAlertStackView.alloc.init;
...
@interface ZYAlertTextFieldMaker ()
@property (nonatomic, strong) UIView *textField;
@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) ZYAlertStackView *mainView;
...
@implementation ZYAlertTextFieldMaker
               ...
               mainView:(UIStackView *)mainView
{
    if (self = [super init]) {
        ...
        self.mainView = (ZYAlertStackView *)mainView;
